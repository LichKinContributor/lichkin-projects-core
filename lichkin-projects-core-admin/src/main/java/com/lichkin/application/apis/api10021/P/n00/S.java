...
//		sql.select(SysUserOperLogR.busType);
//		sql.select(SysUserOperLogR.operType);// LKOperTypeEnum
//		sql.select(SysUserOperLogR.requestDatas);
//		sql.select(SysUserOperLogR.requestUrl);
		sql.select(SysUserOperLogR.requestIp);
		sql.select(SysUserOperLogR.requestTime);
		sql.select(SysUserOperLogR.requestId);
//		sql.select(SysUserOperLogR.loginId);
		...
		// 筛选条件（必填项）
//		sql.eq(SysUserOperLogR.compId, compId);
//		sql.eq(SysUserOperLogR.usingStatus, LKUsingStatusEnum.USING);
		...
		if (StringUtils.isNotBlank(busType)) {
			sql.eq(SysUserOperLogR.busType, busType);
		}
		...
		if (StringUtils.isNotBlank(cellphone)) {
			sql.like(SysUserLoginR.cellphone, LikeType.ALL, cellphone);
		}

		final String startDate = sin.getStartDate();
		if (StringUtils.isNotBlank(startDate)) {
			sql.gte(SysUserOperLogR.requestTime, LKDateTimeUtils.toString(LKDateTimeUtils.toDate(startDate, LKDateTimeTypeEnum.DATE_ONLY), LKDateTimeTypeEnum.TIMESTAMP_MIN));
		}

		final String endDate = sin.getEndDate();
		if (StringUtils.isNotBlank(endDate)) {
			sql.lte(SysUserOperLogR.requestTime, LKDateTimeUtils.toString(LKDateTimeUtils.toDate(endDate + " 23:59:59.999", LKDateTimeTypeEnum.TIMESTAMP), LKDateTimeTypeEnum.TIMESTAMP_MIN));
		}
