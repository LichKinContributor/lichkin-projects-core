...
public class S extends LKApiServiceImpl<I, SO> implements LKApiService<I, SO> {
	...
	@Transactional
	@Override
	public SO handle(I sin) throws LKException {
		...
		SysCompEntity comp = compId.equals(LKFrameworkStatics.LichKin) ? superComp() : dao.findOneById(SysCompEntity.class, compId);
		if (comp == null) {
	...
	private SO superAdmin(I sin) {
		...
		sql.where(new Condition(new Condition(new eq(SysMenuR.auth, LKYesNoEnum.YES)), new Condition(false, new isNull(SysMenuR.auth))));
	...
	private SysAdminLoginEntity superAdminLogin() {
		...
		adminLogin.setSuperAdmin(LKYesNoEnum.YES);
		return adminLogin;
	}
